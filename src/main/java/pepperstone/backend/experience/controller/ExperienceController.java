...
public class ExperienceController {
    ...
    @GetMapping("/recent")
    public ResponseEntity<Map<String, Object>> getRecentExperience(@AuthenticationPrincipal UserEntity userInfo) {
        try {
            Optional<UserEntity> user = userRepository.findById(userInfo.getId());
            ...
            return ResponseEntity.ok(Map.of("code", 200, "data", experienceService.getRecentExperience(user.get())));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(Map.of("code", 400, "message", e.getMessage()));
        } catch (RuntimeException e) {
            return ResponseEntity.status(500).body(Map.of("code", 500, "message", "Internal Server Error: 경험치 현황 데이터를 가져오는 도중 오류가 발생했습니다."));
        }
    }
}