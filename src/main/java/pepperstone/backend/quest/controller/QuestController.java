...
public class QuestController {
    ...
    public ResponseEntity<Map<String, Object>> getQuestDetail(@AuthenticationPrincipal UserEntity userInfo,
        ...
        try {
            ...
            final QuestDetailResponseDTO resDTO = questService.getQuestDetails(user, type, questId);

            return ResponseEntity.ok().body(Map.of("code", 200, "data", resDTO));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(Map.of("code", 400, "data", e.getMessage()));
        } catch (RuntimeException e) {
            return ResponseEntity.status(500).body(Map.of("code", 500, "data", "퀘스트 상세 오류. 잠시 후 다시 시도해주세요."));
        }
    }
}
